...
flywheel:
  ...
  content_sha256: 380810c3f642c88b7ec1722f61cd3e6a2797a12c972b023c3a72d45a5030bb09
...
The in-core visualizer is removed at 0.0.9. The viz section, the ~3,700-line embedded page template, the page sources, the bundler, the viz test suite, the browser baselines and the viz-only fixtures all left the repo, and `hypergraph viz` survived as a signpost stub until the 0.1.0 gate removed it: the CLI now carries no viz entry at all [rec: loyal-tide-3608] [rec: steady-ember-8009].

- **What remains in core is the contract, and it was already there.** `export` writes `.hypergraph/cache/{record,state}.json`, and any renderer that reads those files can draw both graphs — backend/local-adapter.md had already named the exports as the whole integration surface, which is why this cut was the first of the planned restructuring: nothing downstream changes. The `viz:` comment blocks left both configs with the stub — core never read the key, and the one remaining sentence in SPEC and README says visualization is external with the exports as the contract [rec: loyal-tide-3608] [rec: steady-ember-8009].
...
- steady-ember-8009 — the stub's removal at the 0.1.0 gate: no viz entry in the CLI at all
