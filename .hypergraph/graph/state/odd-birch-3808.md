...
parents:
- damp-basin-8974
summary: Eight releases live on PyPI, every one verified from the public index. 0.0.13 (named views + the rewritten docs) is the newest; the index goes 0.0.12 → 0.0.13.
flywheel:
  node_id: fc3ca91a-1b5d-5543-921b-b82d222f75ec
  slug: polished-sky-8897
  revision: 6
  pushed_at: '2026-08-19T16:54:06+00:00'
  content_sha256: 7432c3874eb56b8b0834b1f3cafd04abfc108258db0c10ab8de55b99c18f38ee
...
Eight releases are live on PyPI, and every one was verified **from the public index** rather than from `dist/` — the check that distinguishes "built" from "shipped" [rec: patient-ridge-8464] [rec: stormy-glade-0866] [rec: witty-summit-9656].
...
- **0.0.13 is live and index-verified — the eighth release** [rec: weathered-ivy-5352]: named views (`hypergraph views add`, view-qualified impacts, per-view reconciliation) plus the structure-first public docs, staged at [rec: scarlet-dawn-9811] and published via `uv publish`. Verification ran entirely against the index via `uvx --isolated --from hypergraph-protocol==0.0.13`: the entry point reports 0.0.13, `skills install` lands six skills plus the shared references payload whose spec.md reads v0.0.13, the new `views` subcommand resolves, and the JSON API lists exactly the wheel and sdist — and, for the first time, serves a README whose links resolve on the PyPI page. The CHANGELOG heading was dated in the same commit, per its convention.
...
- weathered-ivy-5352 — 0.0.13 published and verified from the public index
